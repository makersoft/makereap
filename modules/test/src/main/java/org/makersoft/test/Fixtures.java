/*
 * @(#)Fixtures.java 2013-1-22 下午1:21:56
 *
...
//        for (Class<? extends ActiveRecord> type : types) {
//            try {
...
//        Class<? extends ActiveRecord>[] types = new Class[classes.size()];
...
//        for (ApplicationClasses.ApplicationClass c : Play.classes.getAssignableClasses(ActiveRecord.class)) {
//            classes.add((Class<? extends ActiveRecord>)c.javaClass);
//        }
//        disableForeignKeyConstraints();
...
//    }
     ...
     */
    @SuppressWarnings("unchecked")
	public static <T> List<T> loadYamlAsList(String name) {
        return (List<T>)loadYaml(name);
    }
    ...
    public static <T> T loadYaml(String name, Class<T> clazz) {
        Yaml yaml = new Yaml(new CustomClassLoaderConstructor(clazz, Fixtures.class.getClassLoader()));
...
//                }
//            }
            InputStream is = Fixtures.class.getClass().getResourceAsStream(name);
            if (is == null) {
                throw new RuntimeException("Cannot load fixture " + name + ", the file was not found");
            }
